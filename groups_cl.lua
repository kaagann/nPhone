...
local jobs = {
    ...
    --     label = "Geri Dönüşüm",
    --     gain = 3,
    --     image = "https://cdn.discordapp.com/attachments/1025484285525901374/1066024181692637226/icons8-full-trash-48.png",
    --     maxUser = 4,
    --     color = "#FFEFE8"
    -- },
    {
        ...
        label = "Kargo",
        gain = 3,
        image = "https://cdn.discordapp.com/attachments/1025484285525901374/1066298192624353300/icons8-truck-48.png",
        maxUser = 4,
        color = "#fff4da"
    },
}
...
RegisterNetEvent('groups:phoneNotification', function(data)
    Notify("jobcenter", data.title, data.text, {timeout = data.timeout})
    -- SendNUIMessage({
...
RegisterNUICallback("getPlayerDetails", function(data, cb)
    QBCore.Functions.TriggerCallback("player-getJobDetails", function(details)    
        print("veri alıyorum")
            cb(details)
...
RegisterNetEvent("updatePlayerDetails", function()
    print("güncelleme zamanı")
    QBCore.Functions.TriggerCallback("player-getJobDetails", function(details)    
            SendReactMessage("setPlayerDetails", details)
...
RegisterNetEvent('qb-phone:client:RefreshGroupsApp', function(Groups, finish)
    ...
    SendReactMessage("refreshApp", Groups)
...
RegisterNetEvent('qb-phone:client:AddGroupStage', function(_, stage)
    inJob = true
    SendReactMessage("addGroupStage", stage)
end)


RegisterCommand("stgver", function()
    TriggerServerEvent("TestGroups")
end)


RegisterNUICallback("jobcenter_getGroupStage", function(data, cb)
    print("veri istiyorlarrr")
    TriggerServerEvent("server:jobcenter_getGroupStage")
end)
...
RegisterNUICallback('jobcenter_getJobs', function(data, cb) --employment
    cb(jobs)
end)
...
RegisterNUICallback("getGroupData", function(data, cb)
    QBCore.Functions.TriggerCallback("getPlayerGroupData", function(group)
        cb(group)
    end, data)
end)