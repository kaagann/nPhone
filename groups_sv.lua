...
local function pNotifyGroup(group, header, msg, icon, colour, length)
    ...
    for _, v in pairs(EmploymentGroup[group].members) do
        print(v.Player)
        TriggerClientEvent("nPhone:client-notify", v.Player, "jobcenter", header, msg)
    end
end exports("pNotifyGroup", pNotifyGroup)


RegisterNetEvent("nPhone:groups-sendNotif", function(group, header, msg, icon, colour, length)
    pNotifyGroup(group, header, msg, icon, colour, length)
end)
...
local function triggerGroupEvent(groupID, name, ...)
    if not groupID then return print("getGroupMembers was sent an invalid groupID :"..groupID) end
    local temp = {}
    for _,v in pairs(EmploymentGroup[groupID].members) do
        print("event atıcam", v.Player)
        TriggerClientEvent(name, v.Player, ...)
    end
end exports('triggerGroupEvent', triggerGroupEvent)
...
local function DestroyGroup(groupID)
    ...
    exports['nPhone']:resetJobStatus(groupID)
...
local function RemovePlayerFromGroup(src, groupID, disconnected)
    ...
    for k,v in pairs(g) do
        if v.Player == src then
            ...
            TriggerClientEvent('qb-phone:client:RefreshGroupsApp', -1, EmploymentGroup)
            if not disconnected then TriggerClientEvent("nPhone:client-notify", src, "jobcenter", "İş Merkezi", "Gruptan ayrıldın.") end
            TriggerClientEvent("updatePlayerDetails", src)
...
function updateJobDone(groupId, stageId, cb)
    if not groupId then return end 
    
    for k, v in pairs(EmploymentGroup[groupId].stage) do
        print(v.id, stageId)
        if v.id == stageId then
            print("naber evladım")
            EmploymentGroup[groupId].stage[k].count = EmploymentGroup[groupId].stage[k].count + 1

            if EmploymentGroup[groupId].stage[k].count >= EmploymentGroup[groupId].stage[k].total then
                EmploymentGroup[groupId].stage[k].isDone = true
                if cb then cb() end
            end
...
end exports("updateJobDone", updateJobDone)
...
RegisterNetEvent("qb-phone:server:jobcenter_CreateJobGroup", function(data)
    ...
    if Players[src] then TriggerClientEvent("nPhone:client-notify", src, "jobcenter", "İş Merkezi", "Zaten bir grupdasın") return end
    if not data or not data.name then return end
    Players[src] = true
    local ID = #EmploymentGroup+1
    local userAvatar = exports["nPhone"]:getPhoneImage(player.PlayerData.citizenid)
    EmploymentGroup[ID] = {
        id = ID,
        status = "WAITING",
        job = data.job,
        ...
        leader = src,
        members = {
            {name = GetPlayerCharName(src), CID = player.PlayerData.citizenid, Player = src, avatar = userAvatar}
        },
        stage = {},
    }
...
RegisterNetEvent("server:jobcenter_getGroupStage", function()
    local src = source
    local groupID = GetGroupByMembers(src)
    print(groupID)
    if not groupID then return print("setJobStatus was sent an invalid groupID :"..groupID) end
    TriggerClientEvent("qb-phone:client:AddGroupStage", source, EmploymentGroup[groupID].status, EmploymentGroup[groupID].stage)
...
RegisterNetEvent('qb-phone:server:jobcenter_JoinTheGroup', function(data)
    ...
    if Players[src] then return TriggerClientEvent("nPhone:client-notify", src, "jobcenter", "İş Merkezi", "Zaten bir gruptasın") end

    local name = GetPlayerCharName(src)
    pNotifyGroup(data.id, "Job Center", name.." Has joined the group", "fas fa-users", "#FFBF00", 7500)
    local userAvatar = exports["nPhone"]:getPhoneImage(player.PlayerData.citizenid)

    EmploymentGroup[data.id].members[#EmploymentGroup[data.id].members+1] = {name = name, CID = player.PlayerData.citizenid, Player = src, avatar = userAvatar}
    EmploymentGroup[data.id].Users += 1
    Players[src] = true
    TriggerClientEvent("nPhone:client-notify", src, "jobcenter", "İş Merkezi", "Gruba Katıldın")
    TriggerClientEvent('qb-phone:client:RefreshGroupsApp', -1, EmploymentGroup)
end)
...
local function CreateGroup(src, name, job)
...
QBCore.Functions.CreateCallback("player-getJobDetails", function(src, cb, args)
    if Players[src] then
        group = GetGroupByMembers(src)
        cb({group = group, src = src})
    else
        cb(nil) 
    end
end)

QBCore.Functions.CreateCallback("getPlayerGroupData", function(source, cb, id)
    cb(EmploymentGroup[id])
end)